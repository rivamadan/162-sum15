...
int kvserver_cache_concurrent_del(void) {
  pthread_rwlock_t *cachelock;
  pthread_t thread;
  ...
  pthread_rwlock_rdlock(cachelock);
  pthread_create(&thread, NULL, kvserver_concurrent_helper, NULL);
  usleep(1000 * SLEEP_TIME); /* Give the del request a chance to run. */
  ASSERT_EQUAL(synch, 0); /* Ensure the del request didn't complete yet. */
  pthread_rwlock_unlock(cachelock);
  pthread_join(thread, NULL);
  ASSERT_EQUAL(synch, 1); /* Ensure that the del request was able to run after the lock was released. */
  return 1;
}
...
int kvserver_simple_put_get_multiple(void) {
  char *retval;
  int ret;
  ret = kvserver_put(&testserver, "mykey1", "myvalue1");
  ret += kvserver_put(&testserver, "mykey2", "myvalue2");
  ret += kvserver_put(&testserver, "mykey3", "myvalue3");
  ret += kvserver_put(&testserver, "mykey4", "myvalue4");
  ret += kvserver_get(&testserver, "mykey1", &retval);
  ASSERT_PTR_NOT_NULL(retval);
  ASSERT_STRING_EQUAL(retval, "myvalue1");
  free(retval);
  ret += kvserver_get(&testserver, "mykey2", &retval);
  ASSERT_STRING_EQUAL(retval, "myvalue2");
  free(retval);
  ret += kvserver_get(&testserver, "mykey3", &retval);
  ASSERT_STRING_EQUAL(retval, "myvalue3");
  free(retval);
  ret += kvserver_get(&testserver, "mykey4", &retval);
...
test_info_t kvserver_tests[] = {
  ...
  {"DEL request cannot complete when a lock is held on cacheset", kvserver_cache_concurrent_del},
  {"Filling to capacity", kvserver_simple_put_get_multiple},
  NULL_TEST_INFO
};
