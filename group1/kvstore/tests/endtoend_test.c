...
// -------- GROUP 1 TESTS ---------
void *endtoend_test_create_client_thread(void *aux) {
  kvmessage_t reqmsg, *respmsg;
  int pass = 1;
  ...
  for (i = 0; i < MAX_THREADS; i++) {
    ...
    reqmsg.key = "key1";
    respmsg = endtoend_send_and_receive(&reqmsg);
    if (respmsg->type != GETRESP || strcmp(respmsg->key, "key1") != 0 || strcmp(respmsg->value, "value1") != 0)
    {
      pass = 0;
    }
...
void endtoend_test_multiple_threads() {
  pthread_t thread;
  pthread_create(&thread, NULL, &endtoend_test_create_client_thread, NULL);
}

int endtoend_test_multithread(void) {
  int pass;
  pthread_t server_thread;
  pthread_create(&server_thread, NULL, &endtoend_server_runner,
      endtoend_test_multiple_threads);
...
test_info_t endtoend_tests[] = {
  ...
  // -------- GROUP 1 TESTS ---------
  {"End to end test placing a key and accessing it with multiple threads", endtoend_test_multithread},
  // -------- GROUP 1 TESTS ---------
  NULL_TEST_INFO
};
