...
int kvcacheset_ref_bit(void) {
  kvcacheset_put(&testset, "key1", "val1");
  ASSERT_EQUAL(REFBIT_OFF, kvcacheset_ref(&testset, "key1"));
  kvcacheset_put(&testset, "key2", "val2");
  ASSERT_EQUAL(REFBIT_OFF, kvcacheset_ref(&testset, "key2"));
  kvcacheset_put(&testset, "key3", "val3");
  ASSERT_EQUAL(REFBIT_OFF, kvcacheset_ref(&testset, "key3"));
  kvcacheset_put(&testset, "key4", "val4");
  ASSERT_EQUAL(REFBIT_OFF, kvcacheset_ref(&testset, "key4"));
  return 1;
}

int kvcacheset_ref_bit_after_access(void) {
  char *retval = NULL;
  kvcacheset_put(&testset, "key1", "val1");
  kvcacheset_get(&testset, "key1", &retval);
  ASSERT_EQUAL(REFBIT_ON, kvcacheset_ref(&testset, "key1"));
  free(retval);
  return 1;
}
// -------- GROUP 1 TESTS ---------
...
test_info_t kvcacheset_tests[] = {
  ...
  // -------- GROUP 1 TESTS ---------
  {"Make sure all ref bits are unset at start", kvcacheset_ref_bit},
  {"Ref bits set after access", kvcacheset_ref_bit_after_access},
  // -------- GROUP 1 TESTS ---------
  NULL_TEST_INFO
};
