...
int kvcache_replace(void) {
  ...
  ret += kvcache_put(&testcache, "mykey3", "myvalue3");
  ret += kvcache_put(&testcache, "mykey4", "myvalue4");
  ret += kvcache_put(&testcache, "mykey5", "myvalue5");
  ret += kvcache_put(&testcache, "mykey6", "myvalue6");
  ret += kvcache_put(&testcache, "mykey7", "myvalue7");
  ret += kvcache_put(&testcache, "mykey8", "myvalue8");
  ret += kvcache_get(&testcache, "mykey5", &retval);
  ASSERT_PTR_NOT_NULL(retval);
  ASSERT_STRING_EQUAL(retval, "myvalue5");
  free(retval);
  ret += kvcache_get(&testcache, "mykey6", &retval);
  ASSERT_STRING_EQUAL(retval, "myvalue6");
  free(retval);
  ret += kvcache_get(&testcache, "mykey7", &retval);
  ASSERT_STRING_EQUAL(retval, "myvalue7");
  free(retval);
  ret += kvcache_get(&testcache, "mykey8", &retval);
  ASSERT_STRING_EQUAL(retval, "myvalue8");
  free(retval);
  ASSERT_EQUAL(ret, 0);
  return 1;
}
...
test_info_t kvcache_tests[] = {
  ...
  {"replacing cache values", kvcache_replace},
  // -------- GROUP 1 TESTS ---------
  NULL_TEST_INFO
};
