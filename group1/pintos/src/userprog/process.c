...
process_execute (const char *file_name)
{
  ...
  strlcpy (exec_copy, file_name, PGSIZE);

  char *exec_file_name;
  char *save_ptr;

  exec_file_name = strtok_r(exec_copy, " ", &save_ptr);

  thread_current()->exec_sema = (struct semaphore*) malloc(sizeof(struct semaphore));
...
start_process (void *file_name_)
{
  ...
  if (success)
    {
      thread_current()->cp->loaded = LOAD_SUCCESS;
      sema_up(thread_current()->parent->exec_sema);
    }
  else
    {
      thread_current()->cp->loaded = LOAD_FAIL;
      sema_up(thread_current()->parent->exec_sema);
      free(args);
      thread_exit ();
    }
  ...
  // ~~~ Added by Group1 ~~~
  struct file* exec_file = filesys_open (args[0]);
  process_add_file(exec_file);
  file_deny_write (exec_file);

  int **arg_address = (int **) malloc (argc * sizeof(int *));
...
process_wait (tid_t child_tid)
{
  ...
  cp->wait_sema = (struct semaphore*) malloc(sizeof(struct semaphore));
